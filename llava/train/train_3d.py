...
class ModelArguments:
    ...
    # 模型名称或本地路径，例如 llama、qwen 等，用于加载预训练语言模型
    ...
    # 指定模型的类名前缀（会自动拼接为 XxxForCausalLM）

    mm_tunable_parts: Optional[str] = field(default=None, metadata={
        "help": 'Could be "mm_mlp_adapter", "mm_vision_resampler", "mm_vision_tower,mm_mlp_adapter,mm_language_model", ...'
    })
    # 指定要微调的多模态模块，可组合使用逗号分隔；若设置，将覆盖 tune_xxx 系列布尔值

    version: Optional[str] = field(default="v0")
    # 模板版本，例如 "v0", "qwen", "llama_v3" 等，对应不同的对话构造策略

    freeze_backbone: bool = field(default=False)
    # 是否冻结语言模型主干（仅训练多模态相关模块）

    tune_mm_mlp_adapter: bool = field(default=False)
    tune_mm_vision_resampler: bool = field(default=False)
    # 控制是否单独训练 projector / resampler（不推荐与 mm_tunable_parts 混用）
    ...
    # 从视觉模型中选取哪一层作为视觉特征输出，-1 表示最后一层

    pretrain_mm_mlp_adapter: Optional[str] = field(default=None)
    mm_projector_type: Optional[str] = field(default="linear")
    # 指定 adapter 的预训练路径与结构类型（如 linear、mlp）

    mm_use_im_start_end: bool = field(default=False)
    mm_use_im_patch_token: bool = field(default=True)
    # 是否使用图像起止标志 / patch token（用于多图像或视频）

    mm_patch_merge_type: Optional[str] = field(default="flat")
    mm_vision_select_feature: Optional[str] = field(default="patch")
    # patch 合并方式与特征选择方式（如 flat / mean, patch / cls）

    mm_resampler_type: Optional[str] = field(default=None)
    # 多模态特征重采样模块的类型（如 perceiver、qformer）
    ...
    # 用于 patch token 的随机 mask 策略

    mm_spatial_pool_stride: Optional[int] = field(default=None)
    mm_spatial_pool_mode: str = field(default="bilinear")
    mm_spatial_pool_out_channels: Optional[int] = field(default=None)
    # 可选的图像空间池化设置（用于 downsample 特征图）

    mm_perceiver_depth: Optional[int] = field(default=3)
    mm_perceiver_latents: Optional[int] = field(default=32)
    mm_perceiver_ff_mult: Optional[float] = field(default=4)
    mm_perceiver_pretrained: Optional[str] = field(default=None)
    # perceiver 结构参数，用于处理图像或视频特征

    mm_qformer_depth: Optional[int] = field(default=3)
    mm_qformer_latents: Optional[int] = field(default=32)
    mm_qformer_pretrained: Optional[str] = field(default=None)
    # qformer 配置（与 perceiver 类似）

    rope_scaling_factor: Optional[float] = field(default=None)
    rope_scaling_type: Optional[str] = field(default=None)
    # 对位置编码（RoPE）进行缩放，支持长文本建模

    s2: Optional[bool] = field(default=False)
    s2_scales: Optional[str] = field(default="336,672,1008")
    # 多分辨率训练（S^2 机制）支持，提供多个 scale 级别

    use_pos_skipping: Optional[bool] = field(default=False)
    pos_skipping_range: Optional[int] = field(default=4096)
    # 是否使用 token 跨步机制，加速长文本 attention（如 FlashAttention 支持）

    mm_newline_position: Optional[str] = field(default="grid")
    delay_load: Optional[bool] = field(default=True)
    # 控制换行位置的处理方式，以及是否延迟加载视觉模块
    ...
    # grounding（目标指向）相关设置，用于 3D 指向/指认任务
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # 缓存模型文件的目录路径，可避免重复下载模型和 tokenizer

    optim: str = field(default="adamw_torch")
    # 优化器类型，默认使用 PyTorch 实现的 AdamW

    remove_unused_columns: bool = field(default=False)
    # 是否自动移除 dataset 中未使用的字段，防止 dataset -> model 输入不匹配

    freeze_mm_mlp_adapter: bool = field(default=False)
    freeze_mm_vision_resampler: bool = field(default=False)
    # 是否冻结视觉相关的多模态模块参数，避免其参与训练

    mpt_attn_impl: Optional[str] = field(default="triton")
    # 如果使用 MPT 模型，指定其 attention 实现（如 triton、torch）

    model_max_length: int = field(
        default=4096,
        metadata={"help": "Maximum sequence length. Sequences will be right padded (and possibly truncated)."},
    )
    # 最大输入序列长度（包括对话 + 图像 token 等），超过将被截断或填充

    double_quant: bool = field(default=True, metadata={
        "help": "Compress the quantization statistics through double quantization."
    })
    # 是否开启 double quantization，用于更进一步压缩量化模型权重

    quant_type: str = field(default="nf4", metadata={
        "help": "Quantization data type to use. Should be one of `fp4` or `nf4`."
    })
    # 量化类型：fp4 或 nf4，主要用于 bitsandbytes 4位量化训练

    bits: int = field(default=16, metadata={
        "help": "How many bits to use."
    })
    # 使用的权重量化位数，支持 16、8、4 等

    # === LoRA 参数配置（仅在 lora_enable=True 时生效）===
    ...
    lora_bias: str = "none"
    # LoRA 超参数配置：
    # r 是低秩矩阵维度，alpha 是缩放系数，dropout 控制正则化
    # weight_path 可加载已有 LoRA 权重，bias 控制是否同时微调 bias

    mm_projector_lr: Optional[float] = None
    mm_vision_tower_lr: Optional[float] = None
    # 可选地为视觉模块和 projector 设置独立的学习率（优先级高于默认 lr）

    # === 样本分组策略（影响 DataLoader 排序，提升训练效率）===
    group_by_task_length: bool = field(default=False)
    # 是否按任务长度分组（多任务混合训练时可平衡不同任务）

    group_by_varlen: bool = field(default=False)
    # 是否启用变长样本分组，以减少 padding 浪费

    group_by_modality_length: bool = field(default=False)
    group_by_modality_length_auto: bool = field(default=False)
    # 是否按模态（图像/视频/文本）长度分组（用于多模态任务）

    auto_find_batch_size: bool = field(default=False)
    # 是否自动寻找最大可用 batch size（可避免 OOM）

    gradient_checkpointing: bool = field(default=True)
    # 启用梯度检查点（减少显存占用，代价是训练速度变慢）

    verbose_logging: bool = field(default=False)
    # 是否打印详细日志信息（包括参数配置、模型结构等）

    attn_implementation: str = field(default="flash_attention_2", metadata={
        "help": "Use transformers attention implementation."
    })
    # attention 实现方式（如 flash_attention_2、eager、sdpa）
...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
        ...
        self.list_data_dict = []
        print("------------------------dataloader--------------------")
        print(data_args.embodiedscan_folder)
        print("-----------------------------------------------------")
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    # 如果用户提供了 RoPE 或 MM 空间池化等参数，则加载原始配置
    if any(
        [
