...
def train():
    ...
    if hasattr(model, "set_lasar_trainable"):
        model.set_lasar_trainable(model, training_args.tune_lasar_modules)
        mprint(
            f"LASAR modules trainable={training_args.tune_lasar_modules}, "
            f"injection={training_args.use_lasar_injection}"
        )
