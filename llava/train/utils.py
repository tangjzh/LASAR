...
def prepare_config_for_training(
    ...
    # Vision tower: always use command-line argument if provided
    # This allows changing vision tower when loading from checkpoint
    old_vision_tower = getattr(config, "vision_tower_cfg", None)
    if old_vision_tower is not None and old_vision_tower != model_args.vision_tower:
        print(f"⚠️  Overriding vision_tower from checkpoint:")
        print(f"   Old: {old_vision_tower}")
        print(f"   New: {model_args.vision_tower}")
    ...
    config.use_lasar_injection = training_args.use_lasar_injection
    config.tune_lasar_modules = training_args.tune_lasar_modules
    # set data args
