...
class VideoProcessor:
    ...
    def preprocess(
        ...
        if hasattr(image_processor, "crop_size"):
            ...
            crop_size = image_processor.size["width"]
        if strategy == "resize":
    ...
    def process_3d_video(
        ...
        video_dict["vggt_img"] = torch.stack([transforms.ToTensor()(img.resize((224, 224), Image.Resampling.BICUBIC)) for img in video_dict["images"]])
...
def merge_video_dict(video_dict_list):
    ...
    for k in video_dict_list[0]:
        if k in ["world_coords", 'images', 'objects']:
            # 找到最长的序列长度
            max_len = max([video_dict[k].shape[0] for video_dict in video_dict_list])
            ...
            new_video_dict[k] = torch.stack(padded_tensors)
        elif k in ['box_input']:
            for video_dict in video_dict_list:
