...
class LlavaMetaModel(ABC):
    ...
    def save_pretrained(self, output_dir, state_dict=None):
        ...
        if self.get_vision_tower():
            ...
            # Check if vision_tower has its own save_pretrained method (e.g., VEMEVisionTower)
            if hasattr(self.vision_tower, 'save_pretrained') and callable(getattr(self.vision_tower, 'save_pretrained')):
                # Use custom save_pretrained (for VEME and other custom towers)
                vision_tower_state_dict = OrderedDict(
                    {k.split("vision_tower.")[-1]: v for k, v in state_dict.items() if "vision_tower" in k}
                )
                ...
                # Fallback to default save (for standard vision towers)
                vision_tower_state_dict = OrderedDict(
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        # Ensure image_features dtype matches LLM dtype to avoid dtype mismatch errors
        # Get the dtype from LLM's parameters (usually float16 for inference)
        llm_dtype = next(self.llm.parameters()).dtype
        image_features = image_features.to(llm_dtype)
        ...
        # Ensure input_embeds dtype matches LLM dtype to avoid dtype mismatch errors
        # Get the dtype from LLM's parameters (usually float16 for inference)
        llm_dtype = next(self.llm.parameters()).dtype
        input_embeds = input_embeds.to(llm_dtype)
    ...
    def generate(
        ...
        **generation_kwargs,
        ...
        # Ensure inputs_embeds dtype matches LLM dtype to avoid dtype mismatch errors
        llm_dtype = next(self.llm.parameters()).dtype
