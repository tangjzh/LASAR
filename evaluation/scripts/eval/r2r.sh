...
TOTAL_CHUNKS="${2:-1}"
IDX_START="${3:-0}"
GPU_LIST="${4:-0}"  # e.g., "0,2,4,6"
LOG_DIR="${5:-$EVAL_DIR/logs}"

if [ -z "$MODEL_PATH" ]; then
    echo "Usage: $0 MODEL_PATH [TOTAL_CHUNKS] [IDX_START] [GPU_LIST] [LOG_DIR]"
    echo "Example: $0 ./ckpt/navila-8b-8f-sft 8 0 \"0,1,2,3\""
    exit 1
fi
...
if [[ "$FAILED" -ne 0 ]]; then
    echo "Error: one or more R2R chunks failed. Check logs in: $LOG_DIR"
    exit 1
fi

echo "All chunks finished. Logs saved to: $LOG_DIR"
