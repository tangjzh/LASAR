...
from habitat.utils.geometry_utils import angle_between_quaternions, quaternion_from_two_vectors
...
class ShortestPathFollowerCompat:
    ...
    def __init__(self, sim: HabitatSim, goal_radius: float, return_one_hot: bool = True):
        ...
        self._mode = (
            "geodesic_path" if getattr(sim, "get_straight_shortest_path_points", None) is not None else "greedy"
        )
    ...
    def _est_max_grad_dir(self, goal_pos: np.array) -> np.array:
        ...
        if self.mode == "geodesic_path":
            ...
            max_grad_dir = quaternion_from_two_vectors(
                ...
                points[1] - points[0] + EPSILON * np.cross(self._sim.up_vector, self._sim.forward_vector),
            )
            max_grad_dir.x = 0
