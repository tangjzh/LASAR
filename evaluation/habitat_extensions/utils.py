...
from habitat.utils.geometry_utils import quaternion_rotate_vector, quaternion_to_list
...
def add_step_stats_on_img(
    ...
    y = 0
    max_width_to_center = max([int(cv2.getTextSize(wt, font, font_size, thickness)[0][0] / 2) for wt in wrapped_text])
...
def waypoint_observations_to_image(
    ...
    if "rgb" in observation:
        ...
        rgb = np.concatenate([horizontal_bar, rgb, stop_prob, horizontal_bar], axis=0).astype(np.uint8)
    ...
    if map_info is not None:
        ...
        if oracle_waypoint is not None:
            maps.draw_oracle_waypoint(top_down_map, oracle_waypoint, meters_per_px, bounds)

        top_down_map = maps.colorize_topdown_map(top_down_map, mask, fog_of_war_desat_amount=0.75)
...
def navigator_video_frame(
    ...
    rgb = [add_id_on_img(rgb[i][:, 80 : (rgb[i].shape[1] - 80), :], str(i)) for i in range(len(rgb))][::-1]
    ...
    inst_white = np.ones((top_down_map.shape[0], rgb.shape[1] - top_down_map.shape[1], 3)) * 255
    ...
    return np.concatenate((rgb, horizontal_white, map_and_inst), axis=0).astype(np.uint8)
