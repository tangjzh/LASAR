...
from habitat_baselines.utils.common import center_crop, get_image_height_width, overwrite_gym_box_shape
...
class CenterCropperPerSensor(ObservationTransformer):
    ...
    def transform_observation_space(
        ...
        for key in observation_space.spaces:
            if key in self.sensor_crops and observation_space.spaces[key].shape[-3:-1] != self.sensor_crops[key]:
                h, w = get_image_height_width(observation_space.spaces[key], channels_last=True)
                logger.info(
                    "Center cropping observation size of %s from %s to %s" % (key, (h, w), self.sensor_crops[key])
                )
...
class ObsStack(ObservationTransformer):
    ...
    def transform_observation_space(
        ...
        for target_uuid, sensors in self.rewrite_dict.items():
            ...
            low = orig_space.low if np.isscalar(orig_space.low) else np.min(orig_space.low)
            high = orig_space.high if np.isscalar(orig_space.high) else np.max(orig_space.high)
            shape = (len(sensors),) + (orig_space.shape)

            observation_space.spaces[target_uuid] = spaces.Box(low=low, high=high, shape=shape, dtype=orig_space.dtype)
