...
class WaypointRewardMeasure(Measure):
    ...
    def reset_metric(self, *args: Any, task: EmbodiedTask, **kwargs: Any) -> None:
        task.measurements.check_measure_dependencies(self.uuid, [DistanceToGoal.cls_uuid, Success.cls_uuid])
        self._previous_distance_to_goal = task.measurements.measures["distance_to_goal"].get_metric()
        self._metric = 0.0
    ...
    def update_metric(self, *args: Any, action: Action, task: EmbodiedTask, **kwargs: Any) -> None:
        reward = self._get_scaled_slack_reward(action)
        reward += self._progress_to_goal(task)
        reward += self._success_reward * task.measurements.measures["success"].get_metric()
        self._metric = reward
    ...
    def __init__(self, *args: Any, sim: Simulator, config: Config, **kwargs: Any):
        ...
        if "{role}" in config.GT_PATH:
            ...
            for role in RxRVLNCEDatasetV1.annotation_roles:
                with gzip.open(config.GT_PATH.format(split=config.SPLIT, role=role), "rt") as f:
...
class TopDownMapVLNCE(Measure):
    ...
    def reset_metric(self, *args: Any, episode: Episode, **kwargs: Any) -> None:
        ...
        self._step_count = 0
        self._metric = None
        self._meters_per_pixel = habitat_maps.calculate_meters_per_pixel(self._map_resolution, self._sim)
        ...
        if self._config.FOG_OF_WAR.DRAW:
            self._fog_of_war_mask = fog_of_war.reveal_fog_of_war(
                ...
                / habitat_maps.calculate_meters_per_pixel(self._map_resolution, sim=self._sim),
            )
        ...
        # MP3D START NODE
        self._nearest_node = maps.get_nearest_node(self._conn_graphs[scene_id], np.take(agent_position, (0, 2)))
        nn_position = self._conn_graphs[self._scene_id].nodes[self._nearest_node]["position"]
    ...
    def get_polar_angle(self) -> float:
        ...
        heading_vector = quaternion_rotate_vector(ref_rotation.inverse(), np.array([0, 0, -1]))
    ...
    def update_map(self, agent_position: List[float]) -> None:
        ...
        gradient_color = 15 + min(self._step_count * 245 // self._config.MAX_EPISODE_STEPS, 245)
        ...
        if self._nearest_node != prev_nearest_node and self._config.DRAW_MP3D_AGENT_PATH:
            nn_position = self._conn_graphs[self._scene_id].nodes[self._nearest_node]["position"]
            ...
            self._top_down_map[
                self.s_x - int(2.0 / 3.0 * point_padding) : self.s_x + int(2.0 / 3.0 * point_padding) + 1,
                self.s_y - int(2.0 / 3.0 * point_padding) : self.s_y + int(2.0 / 3.0 * point_padding) + 1,
            ] = gradient_color
