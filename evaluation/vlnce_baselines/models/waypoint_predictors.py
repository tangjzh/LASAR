...
class WaypointPredictionNet(Net):
    def __init__(self, observation_space: Space, model_config: Config) -> None:
        ...
        self.inst_attn_q = nn.Sequential(nn.Linear(self._hidden_size, dk_inst), nn.ReLU(True))
        ...
        self.text_q_linear = nn.Linear(self.instruction_encoder.output_size, self._hidden_size // 2)
        ...
        d_kv_in = model_config.RGB_ENCODER.output_size + model_config.DEPTH_ENCODER.output_size + ANGLE_FEATURE_SIZE
        ...
        final_feature_size = (
            model_config.RGB_ENCODER.output_size + model_config.DEPTH_ENCODER.output_size + ANGLE_FEATURE_SIZE
        )

        self.stop_linear = nn.Linear(self._hidden_size, 1)
        nn.init.constant_(self.stop_linear.bias, 0)

        self.compress_x_linear = nn.Sequential(nn.Linear(self._hidden_size, final_feature_size), nn.ReLU(True))
    ...
    def distance_to_continuous(self, distance: Tensor) -> Tensor:
        ...
        range_dist = self.wypt_cfg.max_distance_prediction - self.wypt_cfg.min_distance_prediction
    ...
    def num_recurrent_layers(self):
        return self.main_state_encoder.num_recurrent_layers + self.visual_rnn.num_recurrent_layers
    ...
    def forward(
        ...
        rgb_obs = torch.cat(
            [
                observations["rgb"],
                (observations["rgb_history"].permute(1, 2, 3, 0) * masks.squeeze(1)).permute(3, 0, 1, 2).unsqueeze(1),
            ],
            dim=1,
        )
        ...
        rgb_embedding = torch.flatten(rgb_embedding.view(*rgb_size[0:2], *rgb_embedding.shape[1:]), 3)

        # encode depth observations and history
        depth_obs = torch.cat(
            [
                observations["depth"],
                (observations["depth_history"].permute(1, 2, 3, 0) * masks.squeeze(1)).permute(3, 0, 1, 2).unsqueeze(1),
            ],
            dim=1,
        )
        ...
        (visual_hist_feats, rnn_states_out[:, 0 : self.visual_rnn.num_recurrent_layers],) = self.visual_rnn(
        ...
        spatial_attended_rgb = self.rgb_spatial_attn(text_q_spatial, rgb_k_spatial, rgb_v_spatial)  # [B*12, 256]
        ...
        x_small = x_small.unsqueeze(1).repeat(1, attended_visual_features.size(1), 1)  # [B, 12, d]

        dotted_features = (attended_visual_features * x_small).sum(2)
        pano_stop_distribution = CustomFixedCategorical(logits=torch.cat([dotted_features, self.stop_linear(x)], dim=1))
        ...
        if self.wypt_cfg.continuous_distance:
            ...
            distance_variable1 = (
                self.wypt_cfg.max_distance_prediction - self.wypt_cfg.min_distance_prediction
            ) * distance_variable1 + self.wypt_cfg.min_distance_prediction

            distance_variable2 = (
                self.wypt_cfg.max_distance_var - self.wypt_cfg.min_distance_var
            ) * self.distance_var_linear(catted_features).squeeze(2) + self.wypt_cfg.min_distance_var
        else:
            distance_variable1 = self.distance_linear(catted_features)
            distance_variable1 = distance_variable1.squeeze(2)
        ...
        if self.wypt_cfg.continuous_offset:
            offset_variable1 = self.offset_scale * self.offset_linear(catted_features).squeeze(2)
            offset_variable2 = (self.wypt_cfg.max_offset_var - self.wypt_cfg.min_offset_var) * self.offset_var_linear(
                catted_features
            ).squeeze(2) + self.wypt_cfg.min_offset_var
        else:
