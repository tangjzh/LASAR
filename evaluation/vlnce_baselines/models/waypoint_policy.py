...
from vlnce_baselines.models.utils import CustomFixedCategorical, TruncatedNormal, batched_index_select
...
class WaypointPolicy(Policy):
    ...
    def _create_distance_distribution(
        ...
        if self.wypt_cfg.continuous_distance:
            ...
            distance_distribution = CustomFixedCategorical(logits=batched_index_select(var1, dim=1, index=pano))
    ...
    def _create_offset_distribution(
        ...
        if self.wypt_cfg.continuous_offset:
            ...
            offset_distribution = CustomFixedCategorical(logits=batched_index_select(var1, dim=1, index=pano))

        return offset_distribution

    def get_offset_prediction(self, offset_distribution, deterministic: bool = False) -> Tuple[Tensor, Tensor, Tensor]:
        if deterministic:
    ...
    def act(
    ...
    ) -> Tuple[Tensor, List[Dict[str, Any]], Dict[str, Tensor], Dict[str, Tensor], Tensor, Tensor, Tensor,]:
        output = self.net(
            observations,
            rnn_states,
            prev_actions,
        ...
        pano_stop = pano_stop_distribution.mode() if deterministic else pano_stop_distribution.sample()
        stop = (pano_stop == self._config.num_panos).to(torch.uint8)
        pano = pano_stop % self._config.num_panos

        distance_distribution = self._create_distance_distribution(distance_variable1, distance_variable2, pano)
        offset_distribution = self._create_offset_distribution(offset_variable1, offset_variable2, pano)
        ...
        if self.wypt_cfg.predict_distance:
            action_log_probs = action_log_probs + pano_mask * self.wypt_cfg.predict_distance * distance_log_probs
        if self.wypt_cfg.predict_offset:
            action_log_probs = action_log_probs + pano_mask * self.wypt_cfg.predict_offset * offset_log_probs
    ...
    def evaluate_actions(
        ...
        distance_distribution = self._create_distance_distribution(distance_variable1, distance_variable2, idx)
        offset_distribution = self._create_offset_distribution(offset_variable1, offset_variable2, idx)

        # only include distance and offset log probs if the action included them
        pano_mask = (action_components["pano"] != self._config.num_panos).to(pano_log_probs.dtype)
        d_mask = pano_mask * self.wypt_cfg.predict_distance
        o_mask = pano_mask * self.wypt_cfg.predict_offset

        distance_log_probs = d_mask * distance_distribution.log_prob(action_components["distance"])
        offset_log_probs = o_mask * offset_distribution.log_prob(action_components["offset"])
