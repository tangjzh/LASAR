...
class DaggerTrainer(BaseVLNCETrainer):
    ...
    def _update_dataset(self, data_it):
        ...
        if not self.config.MODEL.RGB_ENCODER.trainable:
            ...
            rgb_hook = self.policy.net.rgb_encoder.cnn.register_forward_hook(hook_builder(rgb_features))
        ...
        with tqdm.tqdm(total=self.config.IL.DAGGER.update_size, dynamic_ncols=True) as pbar, lmdb.open(
            self.lmdb_features_dir,
