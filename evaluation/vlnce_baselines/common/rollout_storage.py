...
class ActionDictRolloutStorage(RolloutStorage):
    ...
    def __init__(
        ...
        self.actions = {k: torch.zeros(num_steps, num_envs, 1) for k in ["pano", "offset", "distance"]}
        self.prev_actions = {k: torch.zeros(num_steps + 1, num_envs, 1) for k in ["pano", "offset", "distance"]}
    ...
    def compute_returns(self, next_value: Tensor, use_gae: bool, gamma: float, tau: float) -> None:
        if use_gae:
    ...
    def recurrent_generator(
        ...
        for start_ind in range(0, num_processes, num_envs_per_batch):
            ...
            recurrent_hidden_states_batch = torch.stack(recurrent_hidden_states_batch, 0)
            ...
            for k in self.actions:
                ...
                prev_actions_batch[k] = self._flatten_helper(T, N, prev_actions_batch[k])
