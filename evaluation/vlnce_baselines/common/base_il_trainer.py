...
class BaseVLNCETrainer(BaseILTrainer):
    ...
    def _initialize_policy(
        ...
        self.optimizer = torch.optim.Adam(self.policy.parameters(), lr=self.config.IL.lr)
        if load_from_ckpt:
    ...
    def _eval_checkpoint(
        ...
        prev_actions = torch.zeros(envs.num_envs, 1, device=self.device, dtype=torch.long)
        ...
        log_str = (
            f"[Ckpt: {checkpoint_index}]" " [Episodes evaluated: {evaluated}/{total}]" " [Time elapsed (s): {time}]"
        )
        ...
        while envs.num_envs > 0 and len(stats_episodes) < num_eps:
            ...
            (envs, rnn_states, not_done_masks, prev_actions, batch, rgb_frames,) = self._pause_envs(
                envs_to_pause,
                envs,
                rnn_states,
                not_done_masks,
    ...
    def inference(self) -> None:
        ...
        if self.config.INFERENCE.USE_CKPT_CONFIG:
            config = self._setup_eval_config(self.load_checkpoint(checkpoint_path, map_location="cpu")["config"])
        else:
            config = self.config.clone()
        ...
        config.TASK_CONFIG.TASK.SENSORS = [s for s in config.TASK_CONFIG.TASK.SENSORS if "INSTRUCTION" in s]
        ...
        with tqdm.tqdm(
            ...
            while envs.num_envs > 0:
                ...
                (envs, rnn_states, not_done_masks, prev_actions, batch, _,) = self._pause_envs(
                    envs_to_pause,
                    envs,
                    rnn_states,
                    not_done_masks,
