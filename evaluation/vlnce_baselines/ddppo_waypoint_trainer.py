...
from habitat_extensions.utils import generate_video, waypoint_observations_to_image
...
class DDPPOWaypointTrainer(PPOTrainer):
    ...
    def _set_observation_space(self, envs, batch, instruction_uuid):
        obs_space = apply_obs_transforms_obs_space(envs.observation_spaces[0], self.obs_transforms)
    ...
    def _collect_rollout_step(
    ...
    ) -> Tuple[float, float, int, List[Any], List[bool], DefaultDict[str, List[float]]]:
        pth_time = 0.0
        env_time = 0.0
        ...
        with torch.no_grad():
            ...
            step_prev_actions = {k: v[rollouts.step] for k, v in rollouts.prev_actions.items()}
        ...
        for i in range(self.envs.num_envs):
            if actions[i]["action"] != "STOP":
                ...
                logging_predictions["distance_pred"].append(
                    self.policy.net.distance_to_continuous(action_elements["distance"][i]).item()
                )
                logging_predictions["offset_pred"].append(
                    self.policy.net.offset_to_continuous(action_elements["offset"][i]).item()
                )
                ...
                logging_predictions["offset_var"].append(
                    variances["offset"][i].item() if type(variances["offset"]) is torch.Tensor else variances["offset"]
                )
        ...
        for k, v in self._extract_scalars_from_infos(infos).items():
            if k not in running_episode_stats:
                running_episode_stats[k] = torch.zeros_like(running_episode_stats["count"])
    ...
    def _update_agent(
        ...
        with torch.no_grad():
            ...
            prev_actions = {k: v[rollouts.step] for k, v in rollouts.prev_actions.items()}
    ...
    def train(self) -> None:
        ...
        self.config.TASK_CONFIG.SEED += self.world_rank * self.config.NUM_ENVIRONMENTS
        self.config.freeze()
        ...
        observations = extract_instruction_tokens(observations, instruction_uuid)
        batch = batch_obs(observations, device=self.device)
        ...
        window_logging_predictions = defaultdict(lambda: deque(maxlen=ppo_cfg.reward_window_size))
        ...
        with (
            ...
            for update in range(start_update, self.config.RL.NUM_UPDATES):
                ...
                if ppo_cfg.use_linear_clip_decay:
                    self.agent.clip_param = ppo_cfg.clip_param * linear_decay(update, self.config.RL.NUM_UPDATES)
                ...
                for step in range(ppo_cfg.num_steps):
                    (
                        ...
                        dones,
                        logging_predictions,
                    ) = self._collect_rollout_step(rollouts, current_episode_reward, running_episode_stats)
                    ...
                    if (step >= ppo_cfg.num_steps * self.SHORT_ROLLOUT_THRESHOLD) and int(
                ...
                if len(deltas) > len(ignore):
                    metrics = {k: v / deltas["count"] for k, v in deltas.items() if k not in ignore}
                    writer.add_scalars("metrics", metrics, count_steps)

                if len(window_logging_predictions):
                    preds = {k: np.mean(v) for k, v in window_logging_predictions.items() if len(v)}
                ...
                if update > 0 and update % self.config.RL.LOG_INTERVAL == 0:
                    ...
                    logger.info(
                        "Average window size: {}  {}".format(
                            ...
                            "  ".join(
                                "{}: {:.3f}".format(k, v / deltas["count"]) for k, v in deltas.items() if k != "count"
                            ),
                        )
                    )
    ...
    ) -> Tuple[VectorEnv, Tensor, Dict[str, Tensor], Tensor, Dict[str, Tensor], Dict[str, Tensor],]:
    ...
    def _eval_checkpoint(
        ...
        while envs.num_envs > 0 and len(stats_episodes) < num_eps:
            ...
            prev_actions = {k: v.detach().clone() for k, v in action_elements.items()}

            for i in range(envs.num_envs):
                pano_index = prev_actions["pano"][i].item()
                obs_history["rgb"][i] = batch["rgb"][i][pano_index % batch["rgb"][i].size(0)]
                obs_history["depth"][i] = batch["depth"][i][pano_index % batch["depth"][i].size(0)]

            if config.VIDEO_OPTION and not self.video_in_env:
                for i in range(envs.num_envs):
                    vinfo = deepcopy(infos[i])
                    if "top_down_map_vlnce" not in vinfo:
                        vinfo = envs.call_at(i, "get_info", {"observations": {}})
                    ...
                    frame = waypoint_observations_to_image(
                        observation=obs,
                        info=vinfo,
                        pano_distribution=pano_stop_distribution.probs[i].cpu().numpy(),
                        agent_action_elements={k: v[i].item() for k, v in action_elements.items()},
                        agent_stop=actions[i]["action"] == "STOP",
                        distribution_modes={k: v[i].item() for k, v in distribution_modes.items()},
                        predict_offset=config.MODEL.WAYPOINT.predict_offset,
                        predict_distance=config.MODEL.WAYPOINT.predict_distance,
                        agent_position=batch["globalgps"][i].cpu().unsqueeze(0),
            ...
            for i in range(envs.num_envs):
                ...
                if config.VIDEO_OPTION and not self.video_in_env:
                    generate_video(
                        ...
                        metrics={"SPL": round(stats_episodes[ep_id]["spl"], 5)},
                        tb_writer=writer,
                        fps=1,
                    )
                    rgb_frames[i] = []
            ...
            (envs, rnn_states, prev_actions, not_done_masks, batch, obs_history,) = self._pause_envs(
                envs_to_pause,
                envs,
                rnn_states,
                prev_actions,
        ...
        for k in next(iter(stats_episodes.values())).keys():
            aggregated_stats[k] = sum([v[k] for v in stats_episodes.values()]) / num_episodes
