...
class RecollectTrainer(BaseVLNCETrainer):
    ...
    def train(self) -> None:
        ...
        self.config.IL.RECOLLECT_TRAINER.gt_path = self.config.TASK_CONFIG.TASK.NDTW.GT_PATH
        ...
        if self.config.IL.RECOLLECT_TRAINER.effective_batch_size > 0:
            assert self.config.IL.RECOLLECT_TRAINER.effective_batch_size % self.config.IL.batch_size == 0, (
                "Gradient accumulation: effective_batch_size" " should be a multiple of batch_size."
            )
            ...
            for epoch in range(self.start_epoch, self.config.IL.epochs):
                ...
                for batch_idx in t:
                    ...
                    observations_batch = apply_obs_transforms_batch(
                        {k: v.to(device=self.device, non_blocking=True) for k, v in observations_batch.items()},
                        dataset.obs_transforms,
                    )

                    prev_actions_batch = prev_actions_batch.to(device=self.device, non_blocking=True)
                    not_done_masks = not_done_masks.to(device=self.device, non_blocking=True)
                    corrected_actions_batch = corrected_actions_batch.to(device=self.device, non_blocking=True)
                    weights_batch = weights_batch.to(device=self.device, non_blocking=True)

                    # gradient accumulation
                    if self.config.IL.RECOLLECT_TRAINER.effective_batch_size > 0:
                        loss_accumulation_scalar = (
                            self.config.IL.RECOLLECT_TRAINER.effective_batch_size // self.config.IL.batch_size
                        )
                    ...
                    if self.config.use_pbar:
                        ...
                        if aux_loss != 0.0:
                            aux_s = f" [ActionLoss: {round(action_loss, 4)}]" + f" [AuxLoss: {round(aux_loss, 4)}]"
                        else:
                            aux_s = ""
                        logger.info(
